...
EBPF_OFFLOAD_CONF=$(jq .ebpf_offload? -r < /etc/eni/10-terway.conf | tr '[:upper:]' '[:lower:]')
if [ "$EBPF_OFFLOAD_CONF" = "enable" ]; then
  EBPF_ENABLE=true
fi
...
if [ "$EBPF_ENABLE" = true ]; then
  ...
  if { [ "$KERNEL_MAJOR_VERSION" -eq 4 ] && [ "$KERNEL_MINOR_VERSION" -ge 19 ]; } ||
    ...
    jq 'del(.name,.cniVersion)' < /etc/eni/10-terway.conf | jq '. + {"ebpf_offload": "enable"}' | jq '
{
  "cniVersion": "0.3.0",
  "name": "terway-chainer",
  "plugins": [
