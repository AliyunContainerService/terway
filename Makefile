...
build-push-policy:
	docker buildx build --push --provenance=false --sbom=false --build-arg GIT_VERSION=$(GIT_COMMIT_SHORT) --platform $(BUILD_PLATFORMS) -t $(REGISTRY)/terway:policy-$(GIT_COMMIT_SHORT) -f Dockerfile.policy .

.PHONY: build-push-terway
build-push-terway:
	docker buildx build --push --provenance=false --sbom=false --build-arg GIT_VERSION=$(GIT_COMMIT_SHORT) --platform $(BUILD_PLATFORMS) -t $(REGISTRY)/terway:$(GIT_COMMIT_SHORT) -f Dockerfile .

.PHONY: build-terway-controlplane
build-push-terway-controlplane:
	docker buildx build --push --provenance=false --sbom=false --build-arg GIT_VERSION=$(GIT_COMMIT_SHORT) --platform $(BUILD_PLATFORMS) -t $(REGISTRY)/terway-controlplane:$(GIT_COMMIT_SHORT) -f Dockerfile.controlplane .
