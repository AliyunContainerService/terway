...
func (m *Manager) CreateNetworkInterface(ctx context.Context, trunk bool, vSwitchID string, securityGroups []string, ipCount, ipv6Count int, eniTags map[string]string) (*aliyunClient.NetworkInterface, error) {
	...
	// create new
	result, err := realClient.CreateNetworkInterface(ctx, trunk, vSwitchID, securityGroups, ipCount, ipv6Count, eniTags)
	if err != nil {
		return nil, err
	}
