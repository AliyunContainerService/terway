		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, eni := range node.Status.NetworkInterfaces {
				...
				case aliyunClient.ENIStatusInUse:
					inUseENICount++
					v4p := countValidPrefixes(eni.IPv4Prefix)
					v6p := countValidPrefixes(eni.IPv6Prefix)
					totalV4Prefixes += v4p
					if v6p == 0 {
...
func TestSyncPrefixAllocation_SlotWithPrimaryIP(t *testing.T) {
	...
	totalV4 := countValidPrefixes(eni.IPv4Prefix)
...
func TestSyncPrefixAllocation_MultipleENIsDistribute(t *testing.T) {
	...
	totalExisting := countValidPrefixes(eni1.IPv4Prefix) + countValidPrefixes(eni2.IPv4Prefix)
