...
func TestLocalDelegate_syncNodeCR_RetainENIWithAllocations(t *testing.T) {
	...
	// The prefix should be marked Deleting (blocking new allocations) but still present
	ipamWithAllocs.lock.RLock()
	info, ok := ipamWithAllocs.ipv4PrefixMap["10.0.0.0/28"]
	ipamWithAllocs.lock.RUnlock()
	assert.True(t, ok, "prefix should still exist while pods are allocated")
	assert.Equal(t, networkv1beta1.IPPrefixStatusDeleting, info.status, "prefix should be marked Deleting")
}
