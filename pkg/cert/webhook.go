...
func SyncCert() error {
	...
	// update webhook
	webhook, err := cs.AdmissionregistrationV1().MutatingWebhookConfigurations().Get(context.Background(), name, metav1.GetOptions{})
	if err != nil {
		return err
	}
	...
	newCfg := webhook.DeepCopy()
	for i, hook := range webhook.Webhooks {
		if len(hook.ClientConfig.CABundle) != 0 {
			continue
		}
		// patch ca
		newCfg.Webhooks[i].ClientConfig.CABundle = caCertBytes
	}
	patchBytes, err := json.Marshal(newCfg)
	if err != nil {
		return err
	}
