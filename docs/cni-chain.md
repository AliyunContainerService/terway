...
Plugins chained after Terway must comply with the
[CNI specification](https://www.cni.dev/docs/spec/) and must guarantee
idempotency. The container runtime may invoke `ADD` and `DEL` multiple times
for the same Pod (for example, during sandbox creation or deletion retries),
so a chained plugin that is executed repeatedly for the same container
network must not fail or leave inconsistent state.
...
> Please note that there is no guarantee that custom plugin functionality will
> work. Custom plugins must comply with the CNI specification and guarantee
> idempotency, as described above.
